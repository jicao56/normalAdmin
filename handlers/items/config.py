...
class ItemWebSetting(BaseModel):
    web_name: Optional[str] = Body(None, description='网站名称')
    web_logo: Optional[str] = Body(None, description='网站logo')
    token_expire_time: Optional[int] = Body(None, description='值类型：1-字符串  2-整型  3-浮点型  4-json')
    captcha_required: Optional[int] = Body(None, description='登录是否需要验证码：1-需要；2-不需要')
    captcha_type: Optional[int] = Body(None, description='验证码类型：1-纯数字；2：纯英文；3-数字加英文')
    web_copyright: Optional[str] = Body(None, description='版权信息')


class ItemOutWebSetting(ItemOut):
    data: ItemWebSetting
